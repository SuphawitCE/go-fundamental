...
func main() {
	...
	library.books["Introduction to Go Programming"] = BookEntry{
		total:  3,
		lended: 0,
	}
	library.books["Go In Action"] = BookEntry{
		total:  2,
		lended: 0,
	}
	library.books["Go fundamental"] = BookEntry{
		total:  1,
		lended: 0,
	}
	...
	checkedOut := checkoutBook(&library, "Go In Action", &member)
	fmt.Println("\nCheck out a book:")
	...
	returned := returnBook(&library, "Go In Action", &member)
	fmt.Println("\nCheck in a book:")
	if returned {
		printLibraryBooks(&library)
		printMemberAudits(&library)
